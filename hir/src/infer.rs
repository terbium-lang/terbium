...
#[inline]
const fn prim_ty_relation(p: PrimitiveTy, q: PrimitiveTy) -> Relation {
    match (p, q) {
...
impl<Ctx> UnificationType<Ctx> {
    /// Example: `u32` is a subtype of `u64` because they coerce.
    pub fn relation_to<T>(&self, other: &UnificationType<T>) -> Relation {
        match (self, other) {
            ...
            (Self::Tuple(tys), Self::Tuple(other_tys)) => {
                ...
                for (ty, other_ty) in tys.iter().zip(other_tys.iter()) {
                    relation = relation.merge(ty.relation_to(other_ty));
            ...
            (Self::Func(ftys, frt), Self::Func(gtys, grt)) => {
                ...
                for (fty, gty) in ftys.iter().zip(gtys.iter()) {
                    relation = relation.merge(fty.relation_to(gty));
                    if !relation.compatible() {
                        return Relation::Unrelated;
                    }
                }
                relation.merge(frt.relation_to(grt))
            }
            _ => Relation::Unrelated,
        }
    }

    pub const fn has_same_outer_type<T>(&self, other: &UnificationType<T>) -> bool {
        match (self, other) {
    ...
    pub fn has_unknown(&self, i: usize) -> bool {
        match self {
            Self::Unknown(j) => *j == i,
            Self::Known(_, ts) => ts.iter().any(|ty| ty.has_unknown(i)),
        }
    }
}
...
impl<Ctx> Unification<Ctx> {
    ...
    pub fn map_ctx<U>(self, f: impl FnOnce(Ctx) -> U) -> Unification<U> {
        Unification(self.0, f(self.1))
    }
}
...
impl Unification<Spanned<Expr>> {
    pub fn from_ty_expr(ty: Ty, expr: Spanned<Expr>, counter: &mut usize) -> Self {
        let ty = match (ty, expr.clone()) {
            ...
            (Ty::Array(ty, len), Expr::Array(exprs)) => {
                if let Some(len) = len {
...
impl<Ctx> Unifier<Ctx> {
    ...
    pub fn unify_constraint(&mut self, constraint: Constraint) -> Result<()> {
        match constraint {
            ...
            (U::Known(a, at), U::Known(b, bt)) if a == b => {
                for (a, b) in at.into_iter().zip(bt.into_iter()) {
                    self.unify_constraint((a, b))?;
                }
            }
    ...
    pub fn lower_node(&mut self, node: &mut Node) -> Result<()> {
        match node {
            ...
            Node::Let { pat, ty, value: Some(value) } => {
                ...
                let pat_constraint = (
                    UnificationType::from_ty(pat_ty, &mut counter),
                    UnificationType::from_ty(expr_ty.clone(), &mut counter),
                );

                // constraint: type hint = structure of value
                let ty_constraint = (
                    UnificationType::from_ty(ty.clone(), &mut counter),
                    UnificationType::from_ty(expr_ty, &mut counter),
                );
